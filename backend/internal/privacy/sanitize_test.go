...
func TestSanitizeTraceRemovesSensitiveEvidence(t *testing.T) {
	...
	output, summary, err := SanitizeTrace(input)
	if err != nil {
		t.Fatal(err)
	}
	text := string(output)
...
func TestStorefrontFixtureRemovesModelBoundaryDuplicatesAndInputs(t *testing.T) {
	trace, err := os.ReadFile(filepath.Join("..", "..", "..", "test", "fixtures", "storefront-search-trace.json"))
	if err != nil {
		t.Fatal(err)
	}
	sanitized, summary, err := SanitizeTrace(trace)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSanitizeTraceRejectsInvalidJSON(t *testing.T) {
	if _, _, err := SanitizeTrace(json.RawMessage(`{"trace":`)); err == nil {
