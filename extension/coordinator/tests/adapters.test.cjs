...
const { LegacyPollingCompatibilityAdapter } = require('../compatibility-adapter.js');
...
test('legacy polling exists only through the explicit compatibility adapter', async () => {
  let reads = 0;
  const adapter = new LegacyPollingCompatibilityAdapter({
    maxWaitMs: 10,
    now: () => reads,
    pollIntervalMs: 1,
    read: async () => {
      reads += 1;
      return reads === 2
        ? { id: 'legacy_job', status: 'completed' }
        : { id: 'legacy_job', status: 'running' };
    },
    schedule: (callback) => callback(),
    start: async () => 'legacy_job',
  });

  assert.equal(await adapter.startJob({}), 'legacy_job');
  assert.deepEqual(await adapter.waitForJob('legacy_job'), {
    id: 'legacy_job',
    status: 'completed',
  });
});
