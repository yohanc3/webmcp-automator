    ...
    const handleMessage = async (message, sender = {}) => {
      switch (message?.type) {
        ...
        case 'GET_POLICY_REVIEW_STATE': {
          ...
          return { ok: true, state: { context: { origin, siteScopeId: scopeId, policyRevision: policy?.revision ?? null, requestedScope: 'ambient_learn' }, overrideAudit: stored?.overrideAudit || null, policy, retrySpool: await retryMetadata(origin, scopeId) } };
        }
