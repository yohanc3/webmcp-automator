...
importScripts('manifest-contract.js', 'semantic.js', 'recorder-core.js');
...
const startRecording = async (tabId) => {
  ...
  await tabMessage(tabId, { type: 'RECORDING_START', recordingId: recording.id });
...
const stopRecording = async () => {
  ...
  try {
    const response = await tabMessage(recording.tabId, { type: 'RECORDING_STOP' });
...
const refreshTabs = async (origin) => {
  ...
  await Promise.allSettled(tabs
    ...
    .map((tab) => tabMessage(tab.id, { type: 'REFRESH_ADAPTERS' })));
};
