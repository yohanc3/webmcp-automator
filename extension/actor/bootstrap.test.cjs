...
test('can bind the same durable actor protocol to the isolated replay port', () => {
  const runtime = createRuntime();
  const bridge = bootstrap.createActorBridge({
    actor: {},
    documentObject: { location: { href: 'https://shop.example/' }, title: 'Shop' },
    portName: bootstrap.REPLAY_PORT_NAME,
    runtime,
  });

  bridge.start();

  assert.deepEqual(runtime.names, [bootstrap.REPLAY_PORT_NAME]);
});
