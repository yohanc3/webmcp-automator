(function initializeSourceBootstrap(root, factory) {
  ...
  const waitForJob = async (jobId, signal) => {
    while (!signal?.aborted) {
      const response = await sendLegacyMessage(createMessage(MESSAGE_TYPES.getJob, { jobId }));
      if (!response?.ok) throw new Error(response?.error || 'The adapter job disappeared');
      ...
      await new Promise((resolve) => { root.setTimeout(resolve, 200); });
    }
  ...
  const registerAdapters = async () => {
    ...
    legacyControllers.forEach((controller) => controller.abort());
    legacyControllers.clear();
    const manifestContract = root.WebMcpManifest;
    const matching = response.adapters.filter(({ manifest }) => (
      manifestContract.manifestMatchesLocation(manifest, root.location.href)
    ));
    for (const adapter of matching) {
      const controller = new AbortController();
      legacyControllers.set(adapter.versionId, controller);
      await root.document.modelContext.registerTool({
        name: adapter.manifest.tool.name,
        description: adapter.manifest.tool.description,
        inputSchema: manifestContract.buildInputSchema(adapter.manifest.tool),
        annotations: {
        ...
        execute: async (args, client = {}) => {
          const started = await sendLegacyMessage(createMessage(MESSAGE_TYPES.startJob, {
            adapter,
            args,
            sourceUrl: root.location.href,
          }));
          if (!started?.ok) throw new Error(started?.error || 'Could not start the adapter job');
