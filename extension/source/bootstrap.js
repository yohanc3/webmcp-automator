(function initializeSourceBootstrap(root, factory) {
  ...
  // Compatibility messages are limited to discovery/readiness. Tool execution
  // always uses the named event-driven port above.
  const sendCompatibilityMessage = (message) => sendRuntimeMessage(root.chrome.runtime, message);

  const initialize = async () => {
    getDefaultBridge().start();
    return sendCompatibilityMessage(createMessage(MESSAGE_TYPES.pageReady, {
  ...
  const registerAdapters = async () => {
    if (!root.document.modelContext?.registerTool) {
      await sendCompatibilityMessage(createMessage(MESSAGE_TYPES.webMcpStatus, {
        available: false,
      })).catch(() => {});
      return { available: false, registered: 0 };
    }
    const response = await sendCompatibilityMessage(createMessage(MESSAGE_TYPES.getAdapters, {
      origin: root.location.origin,
    }));
    if (!response?.ok || !Array.isArray(response.actionLists)) {
      await getDefaultBridge().registerActionLists([]);
      throw new Error('Published action-list discovery was unavailable');
    }
    return getDefaultBridge().registerActionLists(response.actionLists);
  };
